...
import { ObservableEvent } from "../tests/Observable";
import AccountGateway from "./infra/gateway/AccountGateway";

const signupForm = ref(new SignupComponentDomain());
let accountId = ref("");
let accountGateway: AccountGateway;
signupForm.value.subscribe(async function (event: ObservableEvent) {
  if (event.name === "submitted") {
    const output = await accountGateway.signup(event.data);
...
<template>
  ...
  <div v-if="signupForm.step === 3">
    ...
    <div v-if="signupForm.isDriver" id="car-plate">
      Car Plate: {{ signupForm.carPlate }}
    </div>
  </div>
